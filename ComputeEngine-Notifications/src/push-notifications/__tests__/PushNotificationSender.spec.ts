...
class TweetAndNotification {
  ...
  public readonly tweet: Notification;
  public readonly notification: PushNotification;

  public constructor(suffix: string, createdAt: Date) {
    this.id = "id_" + suffix;

    this.tweet = new Notification(
      this.id,
      "url_" + suffix,
      new NotificationAuthor("author_name_" + suffix, "author_username_" + suffix),
      createdAt,
      "body_" + suffix
    );

    this.notification = PushNotification.fromNotification(this.tweet);
  }
}
...
describe("PushNotificationSender", () => {
  ...
  it("skips already send tweets", async () => {
    ...
    expect(apple.sendArgs).toEqual([{ notification: send2.notification, deviceTokens: appleTokens }]);
  ...
  it("skips old tweets", async () => {
    ...
    expect(apple.sendArgs).toEqual([{ notification: send1.notification, deviceTokens: appleTokens }]);
