...
describe("MpkDatabase", function () {
  ...
  it("GET Vehicles - empty", async function () {
    ...
    expect(firestore.getOpenDataDocumentCallCount).toEqual(0);
    expect(result).toBeUndefined();
  });

  it("GET Vehicles - data", async function () {
    const { firestore, database } = createDatabase();
    firestore.mpkDocument = { timestamp: "TIMESTAMP", data: data1 };
  ...
  it("SET Vehicles", async function () {
    ...
    expect(firestore.saveOpenDataDocumentCallCount).toEqual(0);
